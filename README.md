...
The phone connects **out** to your Hermes server — works behind any NAT, no port forwarding, no VPN, and no USB cable is required for the connection. Optional USB-host tools let Hermes interact with peripherals attached to the phone through USB-C OTG.
...
| **Python toolset** | `tools/`, `tests/` | Python | Runs on the server. 49 `android_*` tools + WebSocket relay. Also lives in [hermes-agent](https://github.com/NousResearch/hermes-agent) as the production copy |
...
```bash
curl -sSL https://raw.githubusercontent.com/lEWFkRAD/hermes-android/main/install.sh | bash
```
...
Restart hermes — run `/plugins` to verify. Should show: `✓ hermes-android v0.5.1 (49 tools)`
...
publishes a fresh debug APK to the [**Latest Build**](https://github.com/lEWFkRAD/hermes-android/releases/tag/latest-build)
...
- For a USB peripheral, connect it through USB-C OTG, then tap **Authorize USB Device** and approve Android's per-device prompt.
...
1. Get the APK: download `hermes-android-<version>.apk` from the [Latest Build release](https://github.com/lEWFkRAD/hermes-android/releases/tag/latest-build), or build it with `cd hermes-android-bridge && ./gradlew assembleDebug`
...
| `android_usb_devices` | List attached USB/OTG devices, interfaces, and endpoints |
| `android_usb_connections` | List currently claimed USB interfaces |
| `android_usb_authorize` | Request Android permission for an attached USB device |
| `android_usb_connect` | Claim an authorized USB interface |
| `android_usb_disconnect` | Release a claimed USB interface |
| `android_usb_bulk_transfer` | Perform bounded bulk/interrupt I/O |
| `android_usb_control_transfer` | Perform bounded USB control requests |
...
Repository operations are documented in [CONTRIBUTING.md](CONTRIBUTING.md). See
[docs/dependencies.md](docs/dependencies.md) for the complete dependency and
platform inventory, and [docs/store-submission.md](docs/store-submission.md)
for the current private-release, Galaxy Store, and Google Play constraints.
...
- **upstream project**: [raulvidis/hermes-android](https://github.com/raulvidis/hermes-android) — this customized distribution retains the original MIT license and attribution
