...
def start_relay(pairing_code: str, port: int = 0) -> None:
    ...
    with _relay_lock:
        ...
        if state.startup_error is not None:
            _relay_instance = None
            raise RuntimeError(f"Relay failed to start: {state.startup_error}") from state.startup_error
...
def stop_relay() -> None:
    ...
    if (
        state.loop is not None
        and not state.loop.is_closed()
        and state.shutdown_event is not None
    ):
...
async def _serve(state: _RelayState, ready: threading.Event) -> None:
    ...
    ROUTES = {
        ...
        "/mic_start":     "POST",
        "/mic_stop":      "POST",
        "/usb/request_permission": "POST",
        "/usb/connect":   "POST",
        "/usb/disconnect": "POST",
        "/usb/bulk_transfer": "POST",
        "/usb/control_transfer": "POST",
        # READ + WRITE
        "/clipboard":     "BOTH",
    }
