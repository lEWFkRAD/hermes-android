...
class BridgeAccessibilityService : AccessibilityService() {
    ...
    override fun onServiceConnected() {
        ...
        // Accessibility services are system-bound, but that alone does not put
        // this process in Android's started-service state. Keep the local bridge
        // alive with a sticky foreground service for as long as accessibility is
        // enabled; this also lets Android restore it after process reclamation.
        try {
            startForegroundService(Intent(this, BridgeKeepAliveService::class.java))
        } catch (error: RuntimeException) {
            Log.e(TAG, "Unable to start bridge keep-alive service", error)
        }
    }
