...
### Added
- USB-host/OTG device layer: authenticated enumeration, Android permission prompts, interface claiming, detach cleanup, and bounded bulk/interrupt/control transfers.
- Seven `android_usb_*` tools, relay routes, agent guidance, and an on-device USB status/authorization UI.
