...
Then restart hermes-gateway and run `/plugins` to verify — should show `✓ hermes-android v0.5.1 (49 tools)`.
...
**Option A — prebuilt:** download `hermes-android-<version>.apk` from the [Latest Build release](https://github.com/lEWFkRAD/hermes-android/releases/tag/latest-build); install on-device or `adb install hermes-android-*.apk`.
...
6. USB peripherals: connect through USB-C OTG, tap **Authorize USB Device**, and approve Android's per-device prompt (`android_usb_*`)
